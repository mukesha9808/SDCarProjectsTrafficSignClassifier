...
Here is image of traffic sign randomly chosen from training data one from each class
![alt text][image1]


Here is an exploratory visualization of the data set. It is a bar chart showing how the data is distributed for each traffic sign class in training data, validation data and test data
...
As a first step, I decided to augment data in using diffrent steps because accuracy with this data on validation set was not up to mark. I have done following:
* RGB image
* Use YUV image
* Skew image by 10 pixels in any diretion
* Reduce imgae size by 70%
* Zoom image by 20%
* Rotate Image by -15 to 15 degree
* Add gaussian random noise to image

Here is an example of a traffic sign image after converting using each steps mentioned above

![alt text][image3]

As a last step, I normalized the image data to bring mean to 0
